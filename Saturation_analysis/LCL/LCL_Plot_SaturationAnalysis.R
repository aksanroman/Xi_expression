...
results100 <- data.frame(Sample_num = rep(100),read.delim(file="saturationResults_100iterations_100.txt"))
results106 <- data.frame(Sample_num = rep(106),read.delim(file="saturationResults_100iterations_106.txt"))
...
results55_male <- data.frame(Sample_num = rep(55),read.delim(file="saturationResults_100iterations_male_55.txt"))
results59_male <- data.frame(Sample_num = rep(59),read.delim(file="saturationResults_100iterations_male_59.txt"))
...
saturationResults_male <- rbind(saturationResults_male,results10_male, results15_male,results20_male,results25_male, results30_male,results35_male, 
                                results40_male,results45_male, results50_male,results55_male, results59_male)
...
pdf(file = "saturation_plots_NPX_PAR_NPY_100iterations.pdf", width=2.5, height=2.5)
...
ggplot(data=saturationResults, mapping = aes(x=as.factor(Sample_num), y=npx_sig_up_ypair)) + 
        ...
        labs(
                title="NPX-NPY genes with deltaEx > 0, LCLs",
...
ggplot(data=saturationResults, mapping = aes(x=as.factor(Sample_num), y=npx_sig_up_noY)) + 
        ...
        labs(
                title="NPX no Y genes with deltaEx > 0, LCLs",
                y="Number of genes (FDR<0.05)",
                x="Sample size"
        ) + ylim(0,60)
...
ggplot(data=saturationResults, mapping = aes(x=as.factor(Sample_num), y=npx_sig_down)) + 
        ...
        labs(
                title="NPX genes with deltaEx < 0 , LCLs",
                y="Number of genes (FDR<0.05)",
                x="Sample size"
        ) 
...
ggplot(data=saturationResults, mapping = aes(x=as.factor(Sample_num), y=par_sig_up)) + 
        ...
        labs(
                title="Significant PAR genes, LCLs",
                y="Number of genes (FDR<0.05)",
                x="Sample size"
        ) + ylim(0,11)
...
ggplot(data=saturationResults_male, mapping = aes(x=as.factor(Sample_num), y=npy_sig)) + 
        ...
        labs(
                title="NPY genes with deltaEy > 0, LCLs",
                y="Number of genes (FDR<0.05)",
                x="Sample size"
        ) + ylim(0,11)
dev.off()
