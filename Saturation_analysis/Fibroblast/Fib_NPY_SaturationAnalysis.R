...
metadata_fib_male <- metadata[metadata$karyotype %in% c("XXXXY","XXXY","XXY","XXYY","XY","XYY","XYYYY") & metadata$cell_type == "Fib" & metadata$Technical_replicate == "N",]
rownames(metadata_fib_male) <- metadata_fib_male$sample
...
repeat {
  ...
  mySamples <- sample(1:dim(metadata_fib_male)[1], i, replace = FALSE)
  sample_table <- metadata_fib_male[mySamples,]
